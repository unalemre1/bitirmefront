...
@media (min-width: 769px) {
  /*
    When the sidebar is open, it occupies space within the flex container.
    chat-main (flex: 1) will automatically fill the remaining space.
    The 'too far right' issue was likely from adding redundant margin-left.
    By removing margin-left from .shifted-desktop and adding gap to .chat-page,
    we achieve a cleaner layout.
  */
  .chat-main.shifted-desktop {
    /* No margin-left needed here if chat-page has gap and sidebar is flex item */
    margin-left: 0; /* Ensuring it's explicitly 0 to override any other potential rules */
  }

  .chat-page {
    /* Add gap between sidebar and main chat when sidebar is open */
    gap: 0; /* Default no gap */
  }
  .chat-page.show-desktop { /* Apply show-desktop class to chat-page in Vue template to use this */
    gap: 1rem; /* Adjust this value for the desired space between sidebar and chat */
  }
}
/* IMPORTANT: You need to add ':class="{ 'show-desktop': showSidebar }"' to your <div class="chat-page"> element in your template
   For the `gap` to work correctly. Like this:
...
.message.assistant .message-bubble {
  background: var(--bg-color); /* Light background for assistant bubble */
  color: var(--text-color);
...
.message-input:focus {
  ...
  box-shadow: 0 0 0 3px rgba(0, 123, 255, 0.2); /* Use primary-color for focus glow */
}
