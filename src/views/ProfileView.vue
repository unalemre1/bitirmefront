...
// Initialize `profile` with default empty strings for all required fields
...
const profile = ref<UserProfileWithPassword>({
  ...
  surname: '',
  email: '',
  userType: 'user' as UserType, // Default to 'user' or handle based on application logic
  baro_sicil_no: '', // Initialize as empty string, not undefined
  oldPassword: '',
  newPassword: ''
})
...
const fetchProfile = async () => {
  try {
    ...
    // **CRITICAL FIX FOR TS2322:**
    // Ensure all required fields from the API response are treated as strings.
    // If the API can return null/undefined for these, provide a fallback.
    profile.value.id = data.id || '';
    profile.value.name = data.name || '';
    profile.value.surname = data.surname || '';
    profile.value.email = data.email || '';
    profile.value.userType = data.userType || 'user' as UserType; // Fallback for userType
    profile.value.baro_sicil_no = data.baro_sicil_no || ''; // Fallback for baro_sicil_no

    // oldPassword and newPassword are not fetched from API, so they remain as initialized
    // profile.value = { ...profile.value, ...data }; // This is generally fine, but if 'data' has undefined for required fields, it could cause issues.
    // Explicitly assigning properties with fallbacks is safer.
...
const saveProfile = async () => {
  ...
  try {
    ...
    // Yine, burada da API'den gelen verilerin tiplerine dikkat etmek önemlidir.
    profile.value = {
      ...profile.value,
      id: updatedProfile.id || profile.value.id, // Ensure it's never undefined
      name: updatedProfile.name || profile.value.name,
      surname: updatedProfile.surname || profile.value.surname,
      email: updatedProfile.email || profile.value.email,
      userType: updatedProfile.userType || profile.value.userType,
      baro_sicil_no: updatedProfile.baro_sicil_no || profile.value.baro_sicil_no || '',
    };
